...
export const getAllVariations = async (req, res) => {
	try {
		const variations = await Variation.find()
			...
			.populate('discount', 'amount name startDate endDate'); // Chỉ lấy các trường cần thiết của discount
...
export const deleteVariation = async (req, res) => {
	try {
		...
		if (!deletedVariation) return res.status(404).json({ message: 'Variation not found' });
