...
export const handleVNPayPayment = async (req, res) => {
	try {
		...
		if (cart.totalPrice <= 0) {
			return res.status(400).json({ message: 'Giỏ hàng không hợp lệ để thanh toán' });
		}
		...
		if (existingPayment) {
			return res.status(400).json({ message: 'Đơn hàng này đã có giao dịch đang xử lý' });
		}
...
export const handleVNPayReturn = async (req, res) => {
	try {
		...
		if (vnp_ResponseCode === '00') {
			...
			return res.redirect(`${process.env.CLIENT_URL}/payment-success/${payment._id}`);
			...
			return res.redirect(`${process.env.CLIENT_URL}/payment-failed/${payment._id}`);
