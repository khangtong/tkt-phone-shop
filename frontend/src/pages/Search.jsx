...
const Search = () => {
  ...
  const priceRange = useMemo(() => {
    if (selectedPriceRangeId !== 'custom') {
      return (
        PRICE_RANGES.find((r) => r.id === selectedPriceRangeId) ||
        PRICE_RANGES[0]
      );
    }
  ...
  const checkPriceMatch = (variation, currentPriceRange) => {
    ...
    return (
      effectivePrice >= currentPriceRange.min &&
      effectivePrice <= currentPriceRange.max
    );
  };

  const checkRamMatch = (variation, currentSelectedRams) => {
    return (
      currentSelectedRams.length === 0 ||
      currentSelectedRams.includes(variation.ram)
    );
  };
          ...
          {PRICE_RANGES.map((range) => (
            <div key={range.id} className="flex items-center mb-2">
              <input
                ...
                value={range.id}
                checked={
                  selectedPriceRangeId === range.id && range.id !== 'custom'
                }
                onChange={handlePriceRangeChange}
                className="mr-2 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
              />
              <label
                htmlFor={`price_${range.id}`}
        ...
        <div className="flex justify-between items-center mb-4 bg-white p-3 rounded-md shadow-sm">
          <p className="text-gray-700">
            ...
            <span className="font-semibold">
              {sortedProductsToDisplay.length}
            </span>{' '}
            kết quả với từ khoá{' '}
                            ...
                            const targetVariant =
                              product.variation.find(
                                (v) =>
                                  ...
                                  v.color === selectedVariant.color && // Keep color same for simplicity
                                  v.rom === romVal
                              ) ||
                              product.variation.find((v) => v.rom === romVal); // Fallback if color/ram combo doesn't exist
