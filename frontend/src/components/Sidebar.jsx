...
import {
  ...
  FaShoppingCart, // Icon cho quản lý đơn hàng
} from "react-icons/fa";
        ...
        >
          ...
          <FaChevronDown
            className={`transition-transform ${
              isDropdownOpen === "orders" ? "rotate-180" : "rotate-0"
            }`}
          />
        </button>
        {isDropdownOpen === "orders" && (
          <div className="ml-6 space-y-3.5 transition-opacity duration-300 ease-in-out">
            <Link to="/admin/dashboard/order" className="block">
              <button className="flex items-center bg-amber-500 text-white px-4 py-2 w-full rounded-lg hover:bg-amber-600">
